 ...
 */
RootTimeSlideRateScaleProposal::RootTimeSlideRateScaleProposal(StochasticNode<Tree> *n, StochasticNode<double> *o, double a, double p) : Proposal(p),
    tree( n ),
	origin( o ),
	alpha(a)
{
 ...
 * Add a variable.
 */
void RootTimeSlideRateScaleProposal::addRates( std::vector<StochasticNode<double> *> v)
{
...
void RootTimeSlideRateScaleProposal::addRates(StochasticNode<RbVector<double> > *v)
{
 ...
 *
 */
void RootTimeSlideRateScaleProposal::cleanProposal( void )
{
 ...
 */
RootTimeSlideRateScaleProposal* RootTimeSlideRateScaleProposal::clone( void ) const
{
    
    return new RootTimeSlideRateScaleProposal( *this );
}
 ...
 */
const std::string& RootTimeSlideRateScaleProposal::getProposalName( void ) const
{
    static std::string name = "RootTimeSlideScaleProposal";
    
    return name;
}


double RootTimeSlideRateScaleProposal::getProposalTuningParameter( void ) const
{
    return alpha;
}
 ...
 */
double RootTimeSlideRateScaleProposal::doProposal( void )
{
    ...
    double parent_age  = origin->getValue();
    ...
    if (a < 0 || b < 0)
    {
    	...
    	std::cout << parent_age << " -- " << my_age << " -- " << child_Age << std::endl;

    	throw RbException("Woops");
    }
    ...
    for(size_t i = 0; i < branch_rates.size(); ++i )
    {
        ...
        for (size_t j = 0; j < node->getNumberOfChildren(); j++)
        {
            ...
            double child_new_rate = (my_age - a) * stored_rates[i][j] / (my_new_age - a);
    ...
    for(size_t i = 0; i < branch_vectors.size(); ++i )
    {
        ...
        for (size_t j = 0; j < node->getNumberOfChildren(); j++)
        {
            ...
            // now we set the new value
            branch_vectors[i]->getValue()[child_index] = child_new_rate;
 ...
 */
void RootTimeSlideRateScaleProposal::prepareProposal( void )
{
 ...
 */
void RootTimeSlideRateScaleProposal::printParameterSummary(std::ostream &o, bool name_only) const
{
 ...
 */
void RootTimeSlideRateScaleProposal::undoProposal( void )
{
 ...
 */
void RootTimeSlideRateScaleProposal::swapNodeInternal(DagNode *oldN, DagNode *newN)
{
...
void RootTimeSlideRateScaleProposal::setProposalTuningParameter(double tp)
{
	alpha = tp;
}
 ...
 */
void RootTimeSlideRateScaleProposal::tune( double rate )
{
